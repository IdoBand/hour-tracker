...
export class ShiftService {
  ...
  async getSumOfHours(
    workPlacesIdsArray: string[],
    shiftOrBreak: "shift" | "break",
  ) {
    /**
    ...
    const workPlacesDurations = await Promise.all(
      workPlacesIdsArray.map(async (id) => {
        const shifts = await this.shiftDao.getShifts(firstDayOfThisMonth, id);
        const duration = this.calculateTimeMultipleDates(
          shifts,
          "shift",
          firstDayOfThisWeek,
          firstDayOfThisMonth,
  ...
  calculateTimeMultipleDates(
    ...
    if (shiftOrBreak === "shift") {
      for (const event of events) {
        ...
        if (compareAsc(start, firstDayOfThisWeek) >= 0)
          totalPastWeek += hoursDifference;
        // check if the current shift is from the past month
        if (compareAsc(start, firstDayOfThisMonth) >= 0)
          totalPastMonth += hoursDifference;
      }
    }
  ...
  async determineLastShift(workPlaceId: string) {
     ...
     * @returns { null }
     */
    const lastShiftInShiftTable =
      await this.shiftDao.getLastShiftByWorkPlaceId(workPlaceId);
    const lastShiftInWorkPlaceTable =
      await this.workPlaceDao.getLastShift(workPlaceId);
    ...
    if (
      !lastShiftInShiftTable!.shiftStart ||
      !lastShiftInWorkPlaceTable!.lastShift
    ) {
    ...
    if (check > 0) {
      const update = await this.workPlaceDao.updateLastShift(
        lastShiftInShiftTable!.shiftStart,
        workPlaceId,
      );
    }
