...
import { useCalendarRow } from '@/app/(hooks)/useCalendarRow';
...
const EditWorkPlaceForm = ({workPlace, onClose}: EditWorkPlaceFormProps) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                const { 
                    ...
                    employmentEndDate, 
                    lastShift 
                } = TimeHelper.serializeWorkPlaceDates(updatedInfo.employmentStartDate as Date, updatedInfo.employmentEndDate as null, updatedInfo.lastShift as null)
                dispatch(setCurrentWorkPlace(
                    {...updatedInfo,
  ...
  return (
    <form onSubmit={handleSubmit(data => extractFormData(data as WorkPlace))} 
        className='relative flex flex-col gap-4'
    >
        <div className={`${formSectionContainer} mt-7`}>
            <label className='font-semibold'>Name:</label>
            <input type='text'  {...register('name')} defaultValue={currentWorkPlace!.name} />
        </div>
        <div className={`${formSectionContainer}`}>
            <label className='font-semibold'>Currently Employed?</label>
            <input type='checkbox' defaultChecked={currentWorkPlace?.isCurrent} {...register('isCurrent')} />
        </div>
        <div className={`${formSectionContainer}`}>
            <label className='font-semibold'>Paid On Break?</label>
            <input type='checkbox' defaultChecked={currentWorkPlace?.isBreakPaid} {...register('isBreakPaid')}/>
        </div>
        <div className={`${formSectionContainer}`}>
        ...
        <div className={`${formSectionContainer} mb-7`}>
            ...
            <span className='font-semibold'>Wage Per Hour: </span>
                <span><input {...register('wagePerHour', {required: true})} type='number' min={0} defaultValue={workPlace.wagePerHour} className='w-14 appearance-none outline-none p-[2px] rounded-md'/>{`₪`}</span>
            </span>
        </div>
