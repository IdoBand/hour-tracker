...
const About = ({dict}: AboutPros) => {
  ...
  return (
    <>
      ...
      >
        <div className={`${navbarLinkTextClassName}`}>{dict.about}</div>
      </button>
      {about && (
    ...
    <main className="flex flex-col items-center px-16 py-10 gap-4 md:px-6 md:py-6">
      ...
      <div className={`flex flex-col items-start justify-start w-max`}>
        {tools.map((tool) => {
          return (
            <div
              key={tool.name}
              className="flex gap-2 mb-4 text-xl lg:text:lg md:text-base"
            >
              {tool.icon}
              {tool.name}
            </div>
      ...
      <div className="">
        ...
        <div className={flexCenter}>
          {contacts.map((icon, idx) => {
            return (
              <Link
                ...
                target="_blank"
                key={idx}
                className={`w-8 hover:scale-110 ${icon.className}`}
              >
                {icon.icon}
              </Link>
            );
          ...
          <Link
            href={"https://idoband.onrender.com/"}
            className="hover:text-blue-500 md:ml-2 hover:scale-110"
            target="_blank"
          >
            Portfolio
          </Link>
