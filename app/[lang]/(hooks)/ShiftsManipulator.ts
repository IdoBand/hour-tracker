...
class ShiftsManipulatorClass {
  ...
  filterShiftsByMonthAndYear(shifts: Shift[], date: Date) {
    return shifts.filter((shift) => {
      if (
        isSameMonth(shift.shiftStart as Date, date) &&
        isSameYear(shift.shiftStart as Date, date)
      ) {
        return shift;
      }
    });
  }
  prepareShiftsDatesForTotalCalculation(
    shifts: Shift[],
  ): { start: Date; end: Date }[] {
  ...
  prepareBreakDatesForTotalCalculation(
    shifts: Shift[],
  ): { start: Date; end: Date }[] {
    const startAndEndTimes = shifts
  ...
  calculateSalary(shifts: Shift[]): string {
    ...
    if (shifts.length) {
      for (const shift of shifts) {
        let currentShiftSalary =
          TimeHelper.calculateHoursTwoDates(
